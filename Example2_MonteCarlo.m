...
delta1_all = 0.30*randn(Monte_Carlo,1);
delta2_all = 0.10*randn(Monte_Carlo,1);
...
for j = 1:Monte_Carlo
    [t_plot_j, yy1, yy2] = local_example2_one_run(delta1_all(j),delta2_all(j),1000+j);

    if j == 1
        t_plot = t_plot_j;
        final_Error1 = zeros(Monte_Carlo,length(yy1));
        final_Error2 = zeros(Monte_Carlo,length(yy2));
    end
...
function [t_plot, yy1, yy2] = local_example2_one_run(delta1,delta2,seed_value)
...
for it = 1:M_t
    for iz = 1:N_z
        ...
        xhbar1(it,iz) = (5+delta1)*sin(pi*c);
        xhbar2(it,iz) = (1+delta2)*sin(pi*c);
        ...
        yhbar1(it,iz) = (5+delta1)*pi*cos(pi*c);
        yhbar2(it,iz) = (1+delta2)*pi*cos(pi*c);

        yybar1(it,iz)  =  5*pi^2*sin(pi*c);
        yybar2(it,iz)  =  1*pi^2*sin(pi*c);
        yyhbar1(it,iz) = -(5+delta1)*pi^2*sin(pi*c);
        yyhbar2(it,iz) = -(1+delta2)*pi^2*sin(pi*c);
    end
end
